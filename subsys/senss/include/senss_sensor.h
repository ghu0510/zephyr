...
#define SENSS_SENSOR_DT_DEFINE(node_id, reg_ptr, ctx_ptr, api_ptr)			\
	static struct senss_sensor_ctx							\
		_CONCAT(__senss_sensor_ctx_, Z_DEVICE_DT_DEV_NAME(node_id)) = {		\
...
/**
 * @brief Sensor value convert API
 * @addtogroup sensor_val_convert
 * @{
 */
...
/**
 * @brief Helper function for converting radians to micro degrees.
 *
 * @param rad A pointer to a sensor_value struct, holding the value in radians.
 *
 * @return The converted value, in micro degrees.
 */
static inline int32_t sensor_rad_to_udegrees(const struct sensor_value *rad)
{
	int64_t micro_rad_s = rad->val1 * 1000000LL + rad->val2;

	return (micro_rad_s * 180LL * 1000000LL) / SENSOR_PI;
}
