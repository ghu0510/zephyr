...
SensorState sensors[] = {
	{
		...
		.interval = Milliseconds(100).toRawNanoseconds(),
	...
	{
		.type = CHRE_SENSOR_TYPE_HINGE_ANGLE,
		.sensorIndex = 0,
		.handle = 0,
		.isInitialized = false,
		.enable = kEnableDefault,
...
void handleTimerEvent(const void *eventData) {
	for (size_t i = 0; i < ARRAY_SIZE(sensors); i++) {
		...
		if (sensor.isInitialized && sensor.enable != enable) {
			...
			LOG_INF("Configure [enable %d, status %d]: %s", enable, status,
...
bool nanoappStart() {
	for (size_t i = 0; i < ARRAY_SIZE(sensors); i++) {
		...
		LOG_INF("Sensor %zu initialized: %s with handle %" PRIu32, i,
		...
		if (sensor.isInitialized) {
			...
			if (infoStatus) {
				LOG_INF("SensorInfo: %s, Type=%" PRIu8
					" OnChange=%d OneShot=%d Passive=%d "
					"minInterval=%" PRIu64 "nsec",
					info.sensorName, info.sensorType, info.isOnChange,
					info.isOneShot, info.supportsPassiveMode,
					info.minInterval);
			} else {
				LOG_ERR("chreGetSensorInfo failed");
			}
			...
			if (sensor.enable) {
				...
				bool status = chreSensorConfigure(sensor.handle,
					sensor.info.isOneShot ?
						CHRE_SENSOR_CONFIGURE_MODE_ONE_SHOT :
						CHRE_SENSOR_CONFIGURE_MODE_CONTINUOUS,
					sensor.interval, sensor.latency);
				LOG_INF("Requested data: odr %f Hz, latency %f sec, %s",
...
void nanoappHandleEvent(uint32_t senderInstanceId, uint16_t eventType,
	...
	case CHRE_EVENT_SENSOR_GYROSCOPE_DATA: {
		...
		LOG_INF("%s, %d samples: %f %f %f, accuracy: %u, t=%" PRIu64 " ms",
		...
		if (eventType == CHRE_EVENT_SENSOR_UNCALIBRATED_GYROSCOPE_DATA) {
			LOG_INF("UncalGyro time: first %" PRIu64 " last %" PRIu64 " chre %" PRIu64
	...
	case CHRE_EVENT_SENSOR_HINGE_ANGLE_DATA: {
		...
		LOG_INF("%s, %d samples: %f, accuracy = %u, t=%" PRIu64 " ms",
	...
	case CHRE_EVENT_SENSOR_STATIONARY_DETECT_DATA: {
		...
		LOG_INF("%s, %d samples, accuracy: %u", getSensorName(header.sensorHandle),
	...
	case CHRE_EVENT_SENSOR_SAMPLING_CHANGE: {
		...
		LOG_INF("Sampling Change: handle %" PRIu32 ", status: interval %" PRIu64
	...
	case CHRE_EVENT_TIMER:
		if (!kBreakIt) {
			LOG_ERR("Timer event received with gBreakIt is disabled");
		} else {
	...
	default:
		LOG_WRN("Unhandled event %d", eventType);
		break;
	}
}
